 ...
 */
package org.kie.workbench.common.services.backend.util;
...
public class MavenOutputConverter {
        ...
        if (mavenOutput.size() > 0) {
            ...
            for (int i = 0; i < mavenOutput.size(); i++) {
                ...
                for (final InputProcessor filter : filters) {
                    if (item.contains("BUILD FAILURE")) {
                        process = true;
                        continue;
                    }
                    if (process && filter.accept(path, workingDir, item)) {
    ...
    private static abstract class BaseJavaCompilerErrorProcessor extends BaseInputProcessor {
        ...
        public boolean accept(final Path rootPath,
                              final String workingDir,
                              final String value) {
            return value.matches(".*\\s*ERROR\\s+" + getWorkingDir(workingDir) + ".*");
        }
